 ...
 */
package cn.edu.hrbeu.checkgroupSch;
...
public class CheckGroupSchCallBack implements IServerAction {
	/**
	 * 与框架无关的ajax调用
	 */
		...
		try {
			for (Map<String, String> mParams : lstData) {
				...
				if (content == null || content.trim().equals(""))
					continue;
				pstmt = conn
						.prepareStatement("update t_projectinfo set pro_sta_com='同意' where pro_id=?");
				pstmt.setString(1, content);
				pstmt.executeUpdate();
				pstmt.close();
