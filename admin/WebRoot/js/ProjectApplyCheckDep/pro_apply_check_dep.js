...
function testInvokeCallbackMethod2(xmlHttpObj,datasObj)
{
	...
	refreshComponentDisplay('project_apply_check_dep',null,true);//刷新pageid整个页面
	//window.location.href='/WabacusDemo/ShowReport.wx?PAGEID=project_apply_check_dep';//刷新一下此页面，将新插入的记录显示出来
}

function testInvokeCallbackMethod_3(xmlHttpObj,datasObj)
{
	//只对应院系作品审核页面
	//其他页面需要重新写该函数
	alert(xmlHttpObj.responseText);//打印服务器端方法返回的字符串
	...
	//printInvokeParamsData(datasObjArr);
	//refreshComponentDisplay('invokeservermethod_alonepage1',null,true);
	refreshComponentDisplay('project_apply_check_sch',null,true);//刷新pageid整个页面
	//window.location.href='/WabacusDemo/ShowReport.wx?PAGEID=project_apply_check_dep';//刷新一下此页面，将新插入的记录显示出来
}
